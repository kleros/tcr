...
describe("GTCR", async () => {
  ...
  before("setup accounts", async () => {
    ...
    requester = (await getNamedAccounts()).requester;
    challenger = (await getNamedAccounts()).challenger;
    other = (await getNamedAccounts()).other;
    governor = (await getNamedAccounts()).governor;
    governor2 = (await getNamedAccounts()).governor2;
  });
  beforeEach("contract setup", async function () {
    await deployments.fixture(['EnhancedAppealableArbitrator', "GeneralizedTCR"], {
      fallbackToGlobal: true,
      keepExistingDeployments: false,
    });
   let arbitrator = await ethers.getContract('EnhancedAppealableArbitrator');
    ...
    gtcr = await deployments.get("GeneralizedTCR");
  ...
  it("Should set the correct values and fire the event when requesting registration", async () => {
    ...
    const txAddItem = await gtcr.addItem("0xffb43c480000000000000000000000000000000000000000000000000000000000002222", {
      from: requester,
    ...
    assert.equal(txAddItem.logs[1].event, "ItemStatusChange", "The event has not been created");
    assert.equal(txAddItem.logs[1].args._itemID, itemID, "The event has wrong item ID");
  ...
  it("Should set the correct values and create a dispute after the item is challenged and fire 2 events", async () => {
    await gtcr.addItem("0xffb43c480000000000000000000000000000000000000000000000000000000000002222", {
      from: requester,
      value: submitterTotalCost,
    });
    const itemID = await gtcr.itemList(0);
    ...
    const request = await gtcr.getRequestInfo(itemID, 0);
    assert.equal(request[4][2], challenger, "Challenger has not been set up properly");
  ...
  it("Should successfully execute the request if it has not been challenged and fire the event", async () => {
    ...
    assert.equal(txExecute.logs[0].args._requestIndex.toNumber(), 0, "The event has wrong request index");
    assert.equal(txExecute.logs[0].args._roundIndex.toNumber(), 0, "The event has wrong round index");
  ...
  it("Should change the ruling if the loser paid appeal fee while winner did not", async () => {
    const initialRequesterBalance = await web3.eth.getBalance(requester);
    await gtcr.addItem("0x1111224411ffaa2eaf1111224411ffaa2eaf1111224411ffaa2eaf", {
      from: requester,
      value: submitterTotalCost,
    });
    const itemID = await gtcr.itemList(0);
  ...
  it("Should withdraw correct fees if dispute had winner/loser", async () => {
    ...
    await expectRevert.unspecified(gtcr.withdrawFeesAndRewards(requester, itemID, 0, 1, { from: governor }));
  ...
  it("Check various cases of status requirements and the removing request", async () => {
    ...
    assert.equal(item[1].toNumber(), 0, "Item should have status Absent");
    assert.equal(item[2].toNumber(), 3, "The total number of requests is incorrect");
  ...
  it("Only the governor should be allowed to change state variables", async () => {
    await expectRevert(gtcr.changeTimeToChallenge(11, { from: other }), "The caller must be the governor.");
    ...
    await expectRevert(gtcr.changeArbitrator(other, "0xff", { from: other }), "The caller must be the governor.");
    ...
    await expectRevert(
      gtcr.changeMetaEvidence("_registrationMetaEvidence", "_clearingMetaEvidence", { from: other }),
      "The caller must be the governor."
    );
    await gtcr.changeMetaEvidence("_registrationMetaEvidence", "_clearingMetaEvidence", { from: governor2 });
